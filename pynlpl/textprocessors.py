    ...
    for i, c in enumerate(text):
        ...
        if c in string.punctuation or c in WHITESPACE:
            ...
            if (c == '.' or c == ',') and prev.isdigit() and next.isdigit():
                ...
                pass
            elif c not in WHITESPACE and next == c or prev == c: #group clusters of identical punctuation together
                continue
            elif c == '\r' and prev == '\n':
                #ignore
                begin = i+1
                ...
                token = text[begin:i]
                if token: tokens.append(strip_accents(token))

                #if c not in WHITESPACE and c not in list(EOSMARKERS):
