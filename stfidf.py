...
def tfidf_grams(files):
    files = _toText(files)
    print "1-gram"
    cv = CountVectorizer(tokenizer=tokenize, token_pattern="word", min_df=50, strip_accents='ascii', ngram_range=(1,1), stop_words="english")
    cv.fit(files)
    features1 = cv.get_feature_names()
    print "2-gram"
    cv = CountVectorizer(tokenizer=tokenize, token_pattern="word", min_df=30, strip_accents='ascii', ngram_range=(2,2), stop_words="english")
    cv.fit(files)
    features2 = cv.get_feature_names()
    print "3-gram"
    cv = CountVectorizer(tokenizer=tokenize, token_pattern="word", min_df=20, strip_accents='ascii', ngram_range=(3,3), stop_words="english")
    cv.fit(files)
    features3 = cv.get_feature_names()
    print "4-gram"
    cv = CountVectorizer(tokenizer=tokenize, token_pattern="word", min_df=10, strip_accents='ascii', ngram_range=(4,4), stop_words="english")
    cv.fit(files)
    features4 = cv.get_feature_names()
    feat_total = numpy.concatenate((features1, features2, features3, features4), axis=0)
    tfidf = TfidfVectorizer(files, vocabulary=feat_total, tokenizer=tokenize, stop_words="english", strip_accents="ascii")
    tfs = tfidf.fit_transform(files)
    return tfs, tfidf

def vectorizeTFIDF(files, labels):
    list = _toText(files)
    #this can take some time
    tfidf = SupTfidfVectorizer.SupTfidfVectorizer(tokenizer=tokenize, token_pattern="word", stop_words="english", strip_accents="unicode")
    tfs = tfidf.fit_transform(list, labels)
...
if __name__=='__main__':
    ...
    train_tfs, tfidf = vectorizeTFIDF(train, train_label)
